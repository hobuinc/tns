...
def test_comp(tf_output, comp_event, aoi_and_model, db_fill):
...
def test_add(tf_output, add_event, dynamo, aoi_and_model, h3_indices):
    os.environ['AWS_REGION'] = tf_output['aws_region']
    ...
    added_items = get_entries_by_aoi(dynamo, table_name, aoi_and_model)
    assert added_items['Count'] == 3
    for i in added_items['Items']:
        assert i['aoi_and_model']['S'] == aoi_and_model
        assert i['h3_id']['S'] in h3_indices

def test_update(tf_output, db_fill, update_event, aoi_and_model, updated_h3_indices, h3_indices):
    os.environ['AWS_REGION'] = tf_output['aws_region']
    os.environ['SNS_OUT_ARN'] = tf_output['db_add_sns_out']
    os.environ['DB_TABLE_NAME'] = tf_output['table_name']
    table_name = tf_output['table_name']
    ...
    og_items = get_entries_by_aoi(dynamo, table_name, aoi_and_model)
    ...
    updated_items = get_entries_by_aoi(dynamo, table_name, aoi_and_model)
    assert updated_items['Count'] == 2
    for i in updated_items['Items']:
        assert i['aoi_and_model']['S'] == aoi_and_model
        assert i['h3_id']['S'] in updated_h3_indices

def test_delete(tf_output, db_fill, delete_event, aoi_and_model, h3_indices):
    os.environ['AWS_REGION'] = tf_output['aws_region']
    ...
    deleted_items = get_entries_by_aoi(dynamo, table_name, aoi_and_model)
