...
resource "aws_sns_topic" "sns_in" {
  name = "${local.name_prefix}-compare-sns-in"
}
...
resource "aws_sqs_queue" "sqs_in" {
  name                       = "${local.name_prefix}-compare-sqs-input"
...
resource "aws_sqs_queue" "dlq_in" {
  name = "${local.name_prefix}-compare-dlq-in"
}
...
resource "aws_sns_topic" "sns_out" {
  name         = "${local.name_prefix}-compare-sns-output"
  display_name = "${local.name_prefix}-compare-sns-output"
}

resource "aws_sqs_queue" "sqs_out" {
  name = "${local.name_prefix}-compare-sqs-output"
  redrive_policy = jsonencode({
...
resource "aws_sqs_queue" "dlq_out" {
  name = "${local.name_prefix}-compare-dlq-out"
}
