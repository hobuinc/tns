...
resource aws_sns_topic db_comp_sns_in {
    name = "tns_db_comp_sns_input"
    display_name = "tns_db_comp_sns_input"
}
...
resource aws_sqs_queue db_comp_dlq_in {
    name = "tns_db_comp_dlq_in"
}
resource aws_sqs_queue_redrive_allow_policy db_comp_in_redrive_allow {
    queue_url = aws_sqs_queue.db_comp_dlq_in.id

    redrive_allow_policy = jsonencode({
        redrivePermission = "byQueue",
        sourceQueueArns   = [aws_sqs_queue.db_comp_sqs_in.arn]
    })
}
...
resource aws_sns_topic db_comp_sns_out {
    name = "tns_db_comp_sns_output"
    display_name = "tns_db_comp_sns_output"
}
resource aws_sqs_queue db_comp_sqs_out {
    name = "tns_db_comp_sqs_output"
    redrive_policy = jsonencode({
        deadLetterTargetArn = aws_sqs_queue.db_comp_dlq_out.arn
        maxReceiveCount = 10
    })
}
resource aws_sqs_queue db_comp_dlq_out {
    name = "tns_db_comp_dlq_out"
}
resource aws_sqs_queue_redrive_allow_policy db_comp_out_redrive_allow {
    queue_url = aws_sqs_queue.db_comp_dlq_out.id

    redrive_allow_policy = jsonencode({
        redrivePermission = "byQueue",
        sourceQueueArns   = [aws_sqs_queue.db_comp_sqs_out.arn]
    })
}
...
output db_comp_dlq_in_arn {
    value = aws_sqs_queue.db_comp_dlq_in
}
output db_comp_dlq_out_arn {
    value = aws_sqs_queue.db_comp_dlq_out
}